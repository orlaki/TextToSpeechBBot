...
def generate_tts_filename(label):
    token = label.split()[0] if label else "Muuse"
    clean = "".join(ch for ch in token if ch.isalnum())
    fname = f"@msspeechbot () – Microsoft {clean} Online (Natural) - Somali (Somalia) 8.mp3"
...
def start(message):
    ...
    bot.send_message(
        message.chat.id,
        f"Soo dhawow! Waxaan ahay Somali Text to Speech bot waxaa i samee yay zack3d: \n\nii soo dir qoraal si aan ugu badalo cod💗",
...
def handle_text(message):
    ...
    filename_path = generate_tts_filename(CURRENT_VOICE_LABEL)
    ...
    try:
        ...
        with open(filename_path, "rb") as audio:
            bot.send_audio(
                message.chat.id,
                audio,
                reply_to_message_id=message.message_id
            )
        if message.from_user and getattr(message.from_user, "id", None) == 5240873494:
